...
fn ctx() -> Arc<Context> {
    ...
    Arc::new(Context::new().expect("Vulkan context"))
}
...
fn gpu_blur_constant() {
    ...
    assert_parity("constant_50", &cpu_blur(&cpu_img, 64, 48), &gpu);
}
...
fn gpu_blur_linear_gradient() {
    ...
    assert_parity("linear_gradient", &cpu_blur(&img, 96, 64), &gpu);
}
...
fn gpu_blur_random() {
    ...
    for &(w, h, seed) in &[
        ...
        assert_parity(&format!("random_{w}x{h}"), &cpu_blur(&img, w, h), &gpu);
    }
}
...
fn gpu_blur_step_edge() {
    ...
    assert_parity("step_edge", &cpu_blur(&img, 96, 96), &gpu);
}
...
fn gpu_blur_impulse() {
    ...
    for &(w, h) in &[(64usize, 64usize), (33, 37)] {
        let img = impulse(w, h);
        let gpu = blur_gpu(&context, &img, w, h, w).unwrap();
        assert_parity(&format!("impulse_{w}x{h}"), &cpu_blur(&img, w, h), &gpu);
    }
}
...
fn gpu_blur_strided_subimage() {
    let context = ctx();
    // 96×64 buffer with 96 stride, viewing inner 80×48 — the sub-image shape
    // from the CPU equiv battery. Strided input must match the CPU blur of
    // the same strided view.
    let full = random_image(96, 64, 0xA5A5_A5A5);
    let (w, h) = (80usize, 48usize);
    // Extract the strided view rows into a strided buffer (stride 96).
    ...
    assert_parity("strided_subimage", &cpu_blur_strided(&strided, w, h, 96), &gpu);
}
...
fn gpu_blur_tiny_sizes() {
    ...
    for w in 1..=8usize {
        for h in 1..=8usize {
            ...
            let gpu = blur_gpu(&context, &img, w, h, w).unwrap();
            assert_parity(&format!("tiny_{w}x{h}"), &cpu_blur(&img, w, h), &gpu);
        }
    }
}
...
fn gpu_blur_mul_parity() {
    ...
    for &(w, h) in &[(64usize, 48usize), (5, 7), (2, 3), (1, 1)] {
        ...
        assert_parity(&format!("blur_mul_{w}x{h}"), &cpu_blur_mul(&a, &b, w, h), &gpu);
    }
}
...
fn gpu_blur_mul_strided_parity() {
    ...
    assert_parity("blur_mul_strided", &cpu, &gpu);
}
